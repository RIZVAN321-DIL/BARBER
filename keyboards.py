...
def generate_calendar(year, month, blocked_dates):
    """Генерирует календарь на один месяц с кнопками переключения."""
    first_day = datetime(year, month, 1)
    start_weekday = first_day.weekday()  # пн=0, вс=6
    if month == 12:
    ...
    keyboard = []
    # строка с названием месяца и стрелками
