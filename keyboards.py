...
def barbers_keyboard(barbers):
    kb = [[InlineKeyboardButton(text=f"💈 {b[1]}", callback_data=f"barber_{b[0]}")] for b in barbers]
    return InlineKeyboardMarkup(inline_keyboard=kb)

def services_keyboard(services):
    kb = [[InlineKeyboardButton(text=f"✂️ {s[1]} ({s[3]}₽ / {s[2]} мин)", callback_data=f"service_{s[0]}")] for s in services]
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
def slots_keyboard(slots):
    kb = [[InlineKeyboardButton(text=f"⏰ {s}", callback_data=f"slot_{s}")] for s in slots]
    kb.append([InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_date")])
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
def cancel_order_inline(order_id):
    return InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="❌ Отменить запись", callback_data=f"cancel_{order_id}")]])

def admin_orders_list_keyboard(orders):
    from collections import defaultdict
    grouped = defaultdict(list)
    for o in orders:
        grouped[o[3]].append(o)  # o[3] = date
    keyboard = []
    for date_str in sorted(grouped.keys()):
        keyboard.append([InlineKeyboardButton(text=f"📅 {date_str}", callback_data="ignore")])
        for o in grouped[date_str]:
            order_id = o[0]
            time_str = o[4] if isinstance(o[4], str) else o[4]  # уже время
            service = o[2]
            client = o[6]
            barber = o[5]
...
def block_day_confirm_keyboard(date_str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Заблокировать день", callback_data=f"block_confirm_{date_str}")],
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="back_to_menu")]
    ])

def unblock_day_confirm_keyboard(date_str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Открыть день", callback_data=f"unblock_confirm_{date_str}")],
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="back_to_menu")]
    ])