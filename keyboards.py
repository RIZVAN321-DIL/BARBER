...
from config import WORK_SLOTS, BLOCK_SYMBOL, SERVICES, CLIENT_WEBAPP_URL, ADMIN_WEBAPP_URL
...
def main_menu():
    buttons = [
        ...
        [KeyboardButton(text="🌐 Запись через сайт", web_app=WebAppInfo(url=CLIENT_WEBAPP_URL))],
...
def admin_menu():
    buttons = [
        ...
        [KeyboardButton(text="🖥️ Админ-панель", web_app=WebAppInfo(url=ADMIN_WEBAPP_URL))],
        [KeyboardButton(text="📞 Поделиться ботом")]
    ]
    return ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)
...
def service_buttons():
    kb = []
    for s in SERVICES:
        kb.append([InlineKeyboardButton(text=s, callback_data=f"service_{s}")])
