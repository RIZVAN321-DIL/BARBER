...
from config import CLIENT_WEBAPP_URL, ADMIN_WEBAPP_URL
...
def admin_menu():
    ...
    if ADMIN_WEBAPP_URL:
        buttons.append([KeyboardButton(text="🖥️ Админ-панель", web_app=WebAppInfo(url=ADMIN_WEBAPP_URL))])
    buttons.append([KeyboardButton(text="📞 Поделиться ботом")])
    return ReplyKeyboardMarkup(keyboard=buttons, resize_keyboard=True)

def barbers_keyboard(barbers):
    """Клавиатура выбора мастера (inline)"""
    kb = [[InlineKeyboardButton(text=b[1], callback_data=f"barber_{b[0]}")] for b in barbers]
    return InlineKeyboardMarkup(inline_keyboard=kb)

def services_keyboard(services):
    """Клавиатура выбора услуги с ценой и длительностью"""
...
def calendar_keyboard(year, month, blocked_dates=None):
    """Inline-календарь на месяц с пометкой занятых дней"""
