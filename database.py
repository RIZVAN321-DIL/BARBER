...
import logging
from config import TIMEZONE, WORK_SLOTS, SERVICES, SERVICE_DURATIONS
...
def book_slot(date_str, slot, user_id, service, name, phone):
    conn = sqlite3.connect(DB_NAME)
    try:
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM bookings WHERE date=? AND slot=?", (date_str, slot))
        ...
        cur.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        ''', (user_id, service, date_str, slot, name, phone))
        conn.commit()
...
def get_cancelled_orders():
    ...
    cur = conn.cursor()
    cur.execute('''
        SELECT id, original_order_id, client_name, phone, service,
...
def save_cancelled_order(order_id, user_id, name, phone, service, old_date, old_slot, reason):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO cancelled_orders
        (original_order_id, user_id, client_name, phone, service,
         original_date, original_slot, cancelled_at, reason)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    ''', (order_id, user_id, name, phone, service,
...
def cancel_order(order_id, user_id, is_admin=False, move_to_history=True, reason="cancel"):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("SELECT date, slot, user_id, client_name, phone, service FROM orders WHERE id=? AND status='active'", (order_id,))
    row = cur.fetchone()
    if not row:
        conn.close()
        return False, "not_found"
    date_str, slot, owner_id, name, phone, service = row
    ...
    if move_to_history:
        save_cancelled_order(order_id, owner_id, name, phone, service, date_str, slot, reason)
