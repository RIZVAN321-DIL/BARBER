...
def block_day_for_barber(date_str, barber_id):
    ...
    start_h, start_m = map(int, barber[2].split(':'))
    end_h, end_m = map(int, barber[3].split(':'))
    mins = start_h * 60 + start_m
    end_mins = end_h * 60 + end_m
    while mins < end_mins:
        time_str = f"{mins//60:02d}:{mins%60:02d}"
        cur.execute("INSERT OR IGNORE INTO blocked_slots (date, time, barber_id) VALUES (?, ?, ?)",
                    (date_str, time_str, barber_id))
        mins += 60
    conn.commit()
    conn.close()
...
def is_day_blocked(date_str, barber_id):
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("SELECT 1 FROM blocked_slots WHERE date=? AND barber_id=?", (date_str, barber_id))
...
def get_cancelled_orders():
    conn = sqlite3.connect(DB_NAME)
    cur = conn.cursor()
    cur.execute("SELECT id, user_id, (SELECT name FROM services WHERE id=o.service_id), date, time, client_name, phone FROM orders o WHERE status='cancelled' ORDER BY date DESC LIMIT 20")
    rows = cur.fetchall()
    conn.close()
    return rows