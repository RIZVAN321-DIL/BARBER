...
async def calendar_prev(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Ошибка при смене месяца: {e}")
...
async def help_cmd(message: Message):
    ...
    if user_id == ADMIN_ID:
        help_text = (
            ...
            "⛔ **Выходной** – заблокировать все слоты на выбранный день\n"
...
# ---------- АДМИН: БЫСТРАЯ ЗАПИСЬ ПО ЗВОНКУ (с авто-подбором) ----------
...
async def admin_block_day_start(message: Message, state: FSMContext):
    ...
    year = now.year
    month = now.month
    blocked_dates = get_blocked_dates()  # показываем уже заблокированные даты в календаре
...
async def confirm_block_day(callback: CallbackQuery, state: FSMContext):
    ...
    if success:
        ...
        # Показываем обновлённый календарь
        now = now_moscow()
        year = now.year
        month = now.month
        blocked_dates = get_blocked_dates()
...
# ---------- АДМИН: ОТКРЫТЬ ДЕНЬ (РАЗБЛОКИРОВКА) ----------
