...
async def move_booking(order_id, new_date, new_slot, db_conn, cur):
    """Переносит запись на новый слот. Обновляет bookings и orders."""
    # Получаем старую дату и слот
    cur.execute("SELECT date, slot FROM orders WHERE id=?", (order_id,))
    old_date, old_slot = cur.fetchone()
    # Удаляем старый слот из bookings
    cur.execute("DELETE FROM bookings WHERE date=? AND slot=?", (old_date, old_slot))
    # Добавляем новый слот в bookings
...
# ---------- АДМИН: ВЫХОДНОЙ (С ПЕРЕНОСОМ ЗАПИСЕЙ) ----------
...
async def admin_block_day_date(callback: CallbackQuery, state: FSMContext):
    date_str = callback.data.split("_")[1]
    # Проверяем активные записи на эту дату
    ...
    conn.close()
    if bookings:
        # Запоминаем список записей в state
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        # Ищем ближайший свободный слот, начиная с завтра (избегаем блокируемого дня)
    ...
    conn.commit()
    conn.close()
    # После переноса всех записей блокируем день
