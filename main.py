...
# ---------- СТАРТ ----------@dp.message(Command("start"))
...
# ---------- КЛИЕНТ: ЗАПИСАТЬСЯ ----------@dp.message(F.text == "⚡ Записаться")
...
async def admin_date_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    if not free_slots:
        ...
        return    await state.update_data(date=date_str)
    await callback.message.edit_text(f"📅 {format_date(date_str)}\nВыберите час:", reply_markup=time_slots_buttons(free_slots))
...
@dp.callback_query(F.data == "confirm_yes", StateFilter(QuickState.ready))
async def admin_save_booking(callback: CallbackQuery, state: FSMContext):
    ...
    if not is_slot_free(data['date'], data['slot']):
        ...
        await state.clear()
        return
    book_slot(data['date'], data['slot'], 0, data['service'],
...
async def admin_cancel_id(message: Message, state: FSMContext):
    try:        order_id = int(message.text.strip())
    except:
...
# ---------- ПОДЕЛИТЬСЯ ----------@dp.message(F.text == "📞 Поделиться ботом")
...
async def send_reminders():
    """Отправка напоминаний за 24ч и 2ч до записи"""
    now = datetime.now()
    ...
    # 🔔 2 часа (расширенное окно: от 0 до 2 часов до слота)
    for o in get_orders_for_reminder_2h():
        ...
        # ✅ Исправлено: окно от 0 до 2 часов (ранее было 1.9-2.1)
        if 0 < diff_h <= 2:
            try:
...
async def main():
    init_db()
    scheduler = AsyncIOScheduler(timezone='UTC')  # Можно заменить на 'Europe/Moscow'
