...
from config import BOT_TOKEN, ADMIN_ID, MASTER_PHONE, WORK_SLOTS, TIMEZONE, BLOCK_SYMBOL, SERVICES
...
from keyboards import (
    ...
    phone_request_keyboard, cancel_order_inline, confirm_cancel_keyboard,
...
# ---------- ОБРАБОТЧИК КЛИЕНТСКОГО ВЕБ-ПРИЛОЖЕНИЯ ----------
@dp.message(F.web_app_data)
async def client_web_app_booking(message: Message, state: FSMContext):
    ...
    try:
        ...
        if not service or not name or not phone:
            await message.answer("❌ Не все данные заполнены.")
            return
        ...
        for i in range(7):
            ...
            if free_slots:
                slot = free_slots[0]
                success = book_slot(check_date, slot, user_id, service, name, phone)
                if success:
                    await message.answer(
                        f"✅ <b>Вы записаны через веб-приложение!</b>\n"
                        f"📅 {format_date(check_date)} {slot}\n"
                        f"💇 {service}\n"
                        f"👤 {name}\n\n"
                        f"Напомним за 24 и 2 часа."
                    )
                    admin_text = (
                        f"<b>❗️ НОВАЯ ЗАПИСЬ (Mini App)</b>\n"
                        f"━━━━━━━━━━━━━━━━━\n"
                        f"📅 {check_date} {slot}\n"
                        f"💇 {service}\n"
                        f"👤 {name}\n"
                    ...
                    logger.info(f"Запись через Mini App: {user_id}, {check_date} {slot}")
                    found = True
                    break
            if found:
                break
        if not found:
            await message.answer("❌ Нет свободных слотов в ближайшие 7 дней.")
    except Exception as e:
        logger.error(f"Ошибка web_app_data: {e}")
        await message.answer("❌ Ошибка. Попробуйте позже.")

# ---------- ОБРАБОТЧИК АДМИНСКОЙ ВЕБ-ПАНЕЛИ ----------
@dp.message(F.web_app_data)
async def admin_web_app_handler(message: Message, state: FSMContext):
    user_id = message.from_user.id
    if user_id != ADMIN_ID:
    ...
    try:
        ...
        if action == "get_orders":
            ...
            await message.answer(json.dumps({"orders": orders_list}))
        elif action == "cancel_order":
            order_id = data.get("order_id")
            order = get_order_by_id(order_id)
            if order and order[7] == 'active':
                success, _ = cancel_order(order_id, ADMIN_ID, is_admin=True, move_to_history=True, reason="admin_cancel")
                if success:
                    await message.answer(json.dumps({"status": "ok"}))
                else:
                    await message.answer(json.dumps({"status": "error", "msg": "Не удалось отменить"}))
            else:
                await message.answer(json.dumps({"status": "error", "msg": "Заказ не найден"}))
        elif action == "move_order":
            order_id = data.get("order_id")
            new_date = data.get("new_date")
            new_slot = data.get("new_slot")
            if not new_date or not new_slot:
                await message.answer(json.dumps({"status": "error", "msg": "Не указана новая дата/время"}))
                return
            order = get_order_by_id(order_id)
            if not order or order[7] != 'active':
                await message.answer(json.dumps({"status": "error", "msg": "Заказ не активен"}))
                return
            if not is_slot_free(new_date, new_slot):
                await message.answer(json.dumps({"status": "error", "msg": "Слот занят"}))
                return
            save_cancelled_order(order_id, order[1], order[5], order[6], order[2], order[3], order[4], "move")
            success, _ = await move_booking(order_id, new_date, new_slot)
            if success:
                await message.answer(json.dumps({"status": "ok"}))
            else:
                await message.answer(json.dumps({"status": "error", "msg": "Ошибка переноса"}))
        elif action == "get_stats":
            today_cnt = len(get_orders_for_today())
            tomorrow_cnt = len(get_orders_for_tomorrow())
            future_cnt = len(get_all_future_orders())
            await message.answer(json.dumps({"today": today_cnt, "tomorrow": tomorrow_cnt, "future": future_cnt}))
        elif action == "get_cancelled":
            ...
            await message.answer(json.dumps({"cancelled": cancels_list}))
    except Exception as e:
        logger.error(f"Ошибка в админской web_app: {e}")
        await message.answer(json.dumps({"status": "error", "msg": str(e)}))
...
async def help_cmd(message: Message):
    ...
    if user_id == ADMIN_ID:
        help_text = (
            ...
            "🖥️ <b>Админ-панель</b> – веб-интерфейс для управления\n"
        ...
        help_text = (
            "✂️ <b>Помощь</b>\n\n"
            "⚡ <b>Записаться</b> – выбрать услугу, дату, время\n"
            "🌐 <b>Запись через сайт</b> – удобный мини-сайт для быстрой записи\n"
            "📋 <b>Мои записи</b> – посмотреть и отменить активные записи\n"
            "📞 <b>Поделиться</b> – отправить ссылку на бота\n\n"
...
async def admin_confirm_move(callback: CallbackQuery, state: FSMContext):
    ...
    if not order or order[7] != 'active':
        ...
        return
    old_date, old_slot = order[3], order[4]
    save_cancelled_order(order_id, order[1], order[5], order[6], order[2], old_date, old_slot, "move")
    success, _ = await move_booking(order_id, new_date, new_slot)
    if success:
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        if success:
            ...
            save_cancelled_order(order_id, user_id, name, phone, service, old_date, old_slot_, "block_move")
            moved += 1
            try:
...
async def send_reminders():
    ...
    for o in get_orders_for_reminder_2h():
        ...
        if 0 < diff_h <= 2:
            try:
                await bot.send_message(user_id, f"🔔 <b>Скоро запись!</b>\nЧерез ~2 часа, {format_date(date_str)} в {slot}, у вас запись.")
