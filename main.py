...
from config import BOT_TOKEN, ADMIN_ID, MASTER_PHONE, WORK_SLOTS, TIMEZONE, BLOCK_SYMBOL, CACHE_EXPIRE_SECONDS
from database import (
    ...
    block_day, unblock_day, is_day_blocked, move_booking
)
...
# ---------- КЭШ ДЛЯ БЛОКИРОВАННЫХ ДАТ ----------
_blocked_cache = {"data": None, "timestamp": 0}

async def get_blocked_dates(force_refresh=False):
    now_ts = asyncio.get_event_loop().time()
    if not force_refresh and _blocked_cache["data"] is not None and (now_ts - _blocked_cache["timestamp"]) < CACHE_EXPIRE_SECONDS:
    ...
    for i in range(60):
        ...
        for slot in WORK_SLOTS:
            if await is_slot_free(check_date, slot):
                free_count += 1
        if free_count == 0 or await is_day_blocked(check_date):
            blocked.append(check_date)
    _blocked_cache["data"] = blocked
    _blocked_cache["timestamp"] = now_ts
    return blocked
...
async def find_next_free_slot(after_date: str, avoid_date: str = None):
    ...
    for i in range(30):
        ...
        for slot in WORK_SLOTS:
            if await is_slot_free(date_str, slot):
...
async def _refresh_calendar(callback: CallbackQuery, state: FSMContext, year: int, month: int):
    data = await state.get_data()
    # Используем кэш blocked_dates из состояния, если есть, иначе свежий
    ...
    try:
        ...
        logger.error(f"Ошибка edit_reply_markup: {e}")
...
async def calendar_prev(callback: CallbackQuery, state: FSMContext):
    ...
    if month < 1:
        month = 12
        year -= 1
    await _refresh_calendar(callback, state, year, month)
...
async def client_booking(message: Message, state: FSMContext):
    ...
    if await is_user_banned(user_id):
        await message.answer("❌ Вы заблокированы.")
        return
    if await get_active_order_count(user_id) >= 1:
...
async def quick_booking(callback: CallbackQuery, state: FSMContext):
    ...
    for i in range(14):
        ...
        for slot in WORK_SLOTS:
            if not await is_slot_free(check_date, slot):
                continue
...
async def date_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    free_slots = []
    for slot in WORK_SLOTS:
        if not await is_slot_free(date_str, slot):
            continue
...
async def slot_chosen(callback: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    if not await is_slot_free(data['date'], slot):
...
async def confirm_booking(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    user_id = callback.from_user.id
    if not await is_slot_free(data['date'], data['slot']):
        await callback.message.edit_text("❌ Этот час только что заняли или день заблокирован. Начните заново.")
        await state.clear()
        await callback.answer()
        return

    success = await book_slot(data['date'], data['slot'], user_id, data['service'],
...
async def cancel_my_order(callback: CallbackQuery):
    ...
    try:
        ...
        return
    user_id = callback.from_user.id
    success, info = await cancel_order(order_id, user_id, is_admin=False)
    if success:
...
async def admin_all_orders(message: Message):
    if message.from_user.id != ADMIN_ID:
        return
    orders = await get_all_future_orders()
    if not orders:
...
async def admin_save_booking(callback: CallbackQuery, state: FSMContext):
    ...
    success = await book_slot(data['date'], data['slot'], 0, data['service'],
...
async def admin_confirm_cancel(callback: CallbackQuery, state: FSMContext):
    ...
    if not order_id:
        ...
        await state.clear()
        return
    success, owner_id = await cancel_order(order_id, ADMIN_ID, is_admin=True)
    if success:
...
async def admin_block_day_date(callback: CallbackQuery, state: FSMContext):
    ...
    async with get_db() as db:
        cur = await db.execute("SELECT id, user_id, service, slot, client_name, phone FROM orders WHERE date=? AND status='active'", (date_str,))
    ...
    if bookings:
        ...
        for b in bookings:
            text += f"• {b['service']} в {b['slot']} (клиент: {b['client_name'] if b['client_name'] else 'без имени'})\n"
        ...
        success, msg = await block_day(date_str)
        if success:
            await callback.message.edit_text(f"✅ День {format_date(date_str)} полностью заблокирован (выходной).")
            # Очищаем кэш blocked_dates, чтобы при следующем показе обновилось
            _blocked_cache["data"] = None
            now = now_moscow()
            year = now.year
            month = now.month
            blocked_dates = await get_blocked_dates(force_refresh=True)
...
async def move_bookings_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    for book in bookings:
        ...
        if new_date is None or new_slot is None:
            ...
            errors += 1
            continue
        old_date, old_slot = await move_booking(order_id, new_date, new_slot)
        if old_date is None:
            errors += 1
            continue
        try:
        ...
        moved += 1
    # После переноса блокируем день
    success, msg = await block_day(block_date)
    if success:
...
async def confirm_unblock_day(callback: CallbackQuery, state: FSMContext):
    ...
    if not date_str:
        ...
        await state.clear()
        return
    success, msg = await unblock_day(date_str)
    if success:
...
async def stats(message: Message):
    if message.from_user.id != ADMIN_ID:
        return
    today_cnt = len(await get_orders_for_today())
    tomorrow_cnt = len(await get_orders_for_tomorrow())
    future_cnt = len(await get_all_future_orders())
...
async def send_reminders():
    ...
    for o in await get_orders_for_reminder_24h():
    ...
    for o in await get_orders_for_reminder_2h():
        ...
        if 0 < diff_h <= 2:
            try:
                ...
                await mark_reminder_sent(order_id, '2h')
