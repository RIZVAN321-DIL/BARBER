...
def get_blocked_dates():
    """Возвращает список дат (YYYY-MM-DD), которые полностью заняты или заблокированы."""
    now = now_moscow()
    blocked = []
    for i in range(60):
...
# ---------- УНИВЕРСАЛЬНЫЕ КНОПКИ «НАЗАД» И IGNORE ----------
...
async def back_to_date(callback: CallbackQuery, state: FSMContext):
    ...
    if "service" not in data:
        ...
        await state.clear()
        return
    # показать календарь с текущим месяцем
    now = now_moscow()
...
async def admin_block_day_start(message: Message, state: FSMContext):
    if message.from_user.id != ADMIN_ID:
        return
    await state.clear()
    # показать календарь для выбора даты (без заблокированных)
    now = now_moscow()
    year = now.year
    month = now.month
...
@dp.callback_query(AdminBlockDayState.confirm, F.data.startswith("date_"))
async def admin_block_day_date(callback: CallbackQuery, state: FSMContext):
    ...
    # состояние остаётся AdminBlockDayState.confirm
