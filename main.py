...
from database import (
    ...
    block_day_for_barber, unblock_day_for_barber, is_day_blocked,
...
async def admin_cancel_move_list(message: Message):
    ...
    if not orders:
        ...
        return
    await message.answer(
        "📋 <b>Выберите запись для отмены или переноса:</b>",
...
async def admin_move_order(callback: CallbackQuery):
    ...
    for barber in get_all_barbers():
        ...
        for i in range(30):
            ...
            slots = get_free_slots(d, barber_id, 60)
            if slots:
...
async def confirm_move_order(callback: CallbackQuery):
    ...
    cur = conn.cursor()
    cur.execute("UPDATE orders SET date=?, time=? WHERE id=?", (new_date, new_slot, order_id))
    conn.commit()
    conn.close()
...
async def admin_block_day_date(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Заблокировать день {format_date(date_str)} для всех мастеров?",
...
async def admin_unblock_day_date(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        f"Открыть день {format_date(date_str)}?",
